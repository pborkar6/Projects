...
if ((size(colors, 2) ~= 3) && ...
    (size(colors, 2) ~= 4) && ...
    (size(colors, 2) ~= 5))
    error('Colors parameter must have rows of length 3, 4, or 5 (24-bit RGB color, flag for watershed segmentation, and hue constraint)');
end
...
if ((options == 0) || ...
    ...
    layers = kmeans_fv_colorseg(colors, ...
                                input_image, ...
                                fuzziness, ...
                                fill);
...
elseif (options == 2)
    ...
    watershed_assigned = watershed_image;
    
    for p = 1:size(colors, 1)
        if (watershed_layers(p) == false)
            watershed_assigned = or(watershed_assigned, layers(:, :, p));
        end
    end
    
    [~, nearest_assigned_indices] = bwdist(watershed_assigned);
    
    nearest_assigned_indices = [find(~watershed_assigned), ...
                                nearest_assigned_indices(~watershed_assigned)];
    
    for i = 1:size(nearest_assigned_indices, 1)
        [k, l] = ind2sub([M, N], nearest_assigned_indices(i, 2));
        pixel_index = nearest_assigned_indices(i, 1);
        for p = 1:size(colors, 1)
